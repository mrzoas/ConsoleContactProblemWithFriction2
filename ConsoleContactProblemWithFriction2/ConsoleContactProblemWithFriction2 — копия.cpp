...
static void makeATable(double lambda, double mu, double tA[12][7][4]);
...
static void makeATable(double lambda, double mu, double tA[12][7][4])
{
	...
	for (int i = 0; i <= 8; i++)
	{
		for (int j = 0; j <= 6; j++)
		{
			...
			tA[i][j][0] = (2 * mu + lambda) * tC[i][j][0] + mu * tC[i][j][1] / 2;
			...
			tA[i][j][3] = mu * tC[i][j][0] / 2 + (2 * mu + lambda) * tC[i][j][1];
		}
	}
}