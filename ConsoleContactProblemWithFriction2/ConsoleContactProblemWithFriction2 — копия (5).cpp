...
int main()
{
	...
	/**/
	std::ofstream outl;
	outl.open("output_l2.txt");
	std::ofstream outt;
	outt.open("output_t2.txt");
	std::ofstream outug;
	outug.open("output_ug2.txt");
	std::ofstream outu;
	outu.open("output_u2.txt");
	/**/
	...
	{// Метод последовательных приближений
		...
		// нюанс по поводу трения, его нужно пересчитать для полее крупных сеток (и идеале делат это на ходу)
			...
			{// Проходим по строчкам тела
				...
				{// Проходим по точкам тела в строке iy
				 // Определяем тип узла для правильного вычисления интеграла
					...
					if ((nb == 0) && (k == 1) && (iy == 0) && (ix < nw / 3)) pointFP += -120 * ku; //-60
