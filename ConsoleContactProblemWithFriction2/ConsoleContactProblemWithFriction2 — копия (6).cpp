...
int main()
{
	...
	double delta = 0;
	int iter1 = 0; // количество итераций метода релаксации (решение задачи минимизации
	int iter2 = 0; // количество итераций метода Удзавы (уточнение двойственно переменной)
	int iter3 = 0; // количество итераций метода регуляризации (уменьшение шага сетки)
	int iter4 = 0; // количество итераций метода последовательных приближений (уточнение силы трения)
	int iter1h32 = 0; // количество итераций метода релаксации на сетке 1/32
	int iter1h64 = 0; // количество итераций метода релаксации на сетке 1/64
	...
	/**/
	std::ofstream outl;
	outl.open("output_l3_64.txt");
	std::ofstream outt;
	outt.open("output_t3_64.txt");
	std::ofstream outug;
	outug.open("output_ug3_64.txt");
	std::ofstream outu;
	outu.open("output_u3_64.txt");
	/**/
	
	
	iter3 = 0; // с какого шага начинать счет: 0, 1, 2, 3 -> 1/8, 1/16, 1/32, 1/64
	do
	{// Метод последовательных приближений
		
		iter3 = 0;
		int n_ipr = 65;
		const int lastChangeH = 3; // последнее изменение шага сетки 0, 1, 2, 3 -> 1/8, 1/16, 1/32, 1/64
		
		// нюанс по поводу трения, его нужно пересчитать для более крупных сеток (и идеале делат это на ходу)
		gridChange(33, 65, v, vOld, l, g2);
		gridChange(17, 33, v, vOld, l, g2);
		gridChange( 9, 17, v, vOld, l, g2);

		// Обнуление всего кроме трения
		...
		for (ix = 0; ix < nw; ix++)		{ l[ix] = 0;	t[ix] = 0;		s[ix] = 0; }
		for (iA = 0; iA < nFE2; iA++)	{ v[iA] = 0;	v2[iA] = 0;		hg[iA] = 0;		hlrt[iA] = 0;	vk[iA] = 0; }
		for (ix = 0; ix < nw; ix++)		{ tk[ix] = 0;	sk[ix] = 0; }

		
		do
		{// Метод итеративной проксимальной регуляризации
			
			if (iter3 <= lastChangeH)
			{
				for (iA = 0; iA < nFE2; iA++)
					vOld[iA] = v[iA];

				n_ipr = (1 << (iter3 + 3)) + 1; // в соответствии с iter3 получаем 9, 17, 33, 65

				gridChange(n_ipr, (n_ipr - 1) / 2 + 1, v, vOld, l, g2); // Увеличение n_ipr (измельчение сетки)	
			}
			else
			{
				n_ipr = (1 << (lastChangeH + 3)) + 1;
			}
			...
			{// Метод Удзавы
				...
				{// Метод поточечной релаксации
					...
					iter1++;
					if (n_ipr == 33) iter1h32++;
					if (n_ipr == 65) iter1h64++;
			...
			iter3++;
		} while (n_ipr != (1 << (lastChangeH + 3)) + 1);


		if (iter4 == 0)
		{// Можно считать, что решена задача без трения, сохранм результат
			if (outu.is_open())
			{
				for (int ix = 0; ix < nw; ix++)	outu << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] - v[0 * nFE + (nh - 1) * nw + ix] << "," << v[1 * nFE + 1 * nw * nh + ix] - v[1 * nFE + (nh - 1) * nw + ix] << ") ";
				outu << std::endl << std::endl;
				for (int ix = 0; ix < nw; ix++)	outu << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] << "," << v[1 * nFE + 1 * nw * nh + ix] << ") ";
			}
		}
	...
	//for (int ix = 0; ix < nw; ix++)
	//{
	//	printf("(%.10f , %.10f)\n", ix * h + v[0 * nFE + 1 * nw * nh + ix] - v[0 * nFE + (nh - 1) * nw + ix], l[ix]);
	//}

	//for (int ix = 0; ix < nw; ix++)
	//{
	//	printf("(%.10f , %.10f)\n", ix * h + v[0 * nFE + 1 * nw * nh + ix] - v[0 * nFE + (nh - 1) * nw + ix], t[ix]);
	//}
	std::cout << std::endl;
	...
	if (outug.is_open())
	{
		for (int ix = 0; ix < nw; ix++)	outug << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] - v[0 * nFE + (nh - 1) * nw + ix] << "," << v[1 * nFE + 1 * nw * nh + ix] - v[1 * nFE + (nh - 1) * nw + ix] << ") ";
		outug << std::endl << std::endl;
		for (int ix = 0; ix < nw; ix++)	outug << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] << "," << v[1 * nFE + 1 * nw * nh + ix] << ") ";
	}
	if (outl.is_open())
	{
		for (int ix = 0; ix < nw; ix++) outl << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] - v[0 * nFE + (nh - 1) * nw + ix] << "," << l[ix] << ") ";
	}
	if (outt.is_open())
	{
		for (int ix = 0; ix < nw; ix++) outt << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] - v[0 * nFE + (nh - 1) * nw + ix] << "," << t[ix] << ") ";
	}
					...
					else if ((nb == 0) && (iy == nh - 1))
					{
						...
						{ // t - вертикальный скачок
							...
							else
							{
								...
								if (ix == 0 || ix == nw - 1)
								{// поправка на то, что узел может оказаться краевым
									psi -= h * l[ix] / 2;
									psi /= (A[IDX2C(iA, iA, nFE2)] + h * r / 2);
								}
								else
								{
									psi -= h * l[ix];
...
void makeA(int n, double * A, double koeffA[12][7][4])
{
	...
	{// Правая стенка верхнего тела не должна перемещаться вдоль горизонтальной оси
		...
		for (int i = 0; i < nFE2; i++)
		{
			...
			//A[IDX2C(0 * nFE + 0 * nw * nh + nw * iy + 1, i, nFE2)] = 0; // это относится к левой стнке, не забыть удалить
			//A[IDX2C(i, 0 * nFE + 0 * nw * nh + nw * iy + 1, nFE2)] = 0; // это относится к левой стнке, не забыть удалить
		...
		//A[IDX2C(0 * nFE + iy * nw + 1, 0 * nFE + iy * nw + 1, nFE2)] = 1; // это относится к левой стнке, не забыть удалить
			...
			{// Проходим по строчкам тела
				...
				{// Проходим по точкам тела в строке iy
				 // Определяем тип узла для правильного вычисления интеграла
					...
					if ((nb == 0) && (k == 1) && (iy == 0) ) pointFP += -60 * ku; //-60&& (ix < nw / 3)
...
/*
	здесь lambda соответствует \lambda в книге Александрова и Потапова
	а mu соотвествует G в их книге
*/
static void makeATable(double lambda, double mu, double h, double tA[12][7][4])
{
	double tI[4][6][7] = {
	...
	for (int i = 0; i <= 8; i++)
	{
		for (int j = 0; j <= 6; j++)
		{
			...
			tA[i][j][0] = (2 * mu + lambda) * tC[i][j][0] + mu * tC[i][j][1] +tAu[i][j];
			tA[i][j][1] = lambda * tC[i][j][2] + mu * tC[i][j][3];
			// Коэффициенты при x1 и x2 при рассчете компоненты x2
			tA[i][j][2] = mu * tC[i][j][2] + lambda * tC[i][j][3];
			tA[i][j][3] = mu * tC[i][j][0] + (2 * mu + lambda) * tC[i][j][1] +tAu[i][j];
