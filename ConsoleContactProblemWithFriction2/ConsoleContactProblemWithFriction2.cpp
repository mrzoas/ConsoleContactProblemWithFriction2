...
// 2019.04.23 Исправил ошибку с G/2
...
int main()
{
	...
	n = 65; // Резервирование места для разбиения области сеткой 65 на 65
	...
	mu = E / (2 + 2 * nu);      // Эта переменная соответствует G в книге Александрова и Потапова (1990)
	lambda = nu * E / ((1 + nu) * (1 - 2 * nu));
	makeATable(lambda, mu, h, koeffA); // Коэффициенты генерируютя для плоской деформации. Учтена регуляризация
	...
	std::ofstream outl;
	outl.open("output_l.txt");   // Итоговые значения двойственной переменной
	std::ofstream outt;
	outt.open("output_t.txt");   // Значения скачка
	std::ofstream outug;
	outug.open("output_ug.txt"); // Коортидинаты узлов из области контакта для верхнего тела и для нижнего
	std::ofstream outu;
	outu.open("output_u.txt");   // Коортидинаты узлов из области контакта для верхнего тела и для нижнего (без учета силы трения)
	
	int hNumber = 0; // с какого шага начинать счет: 0, 1, 2, 3 -> 1/8, 1/16, 1/32, 1/64
	do
	{// Метод последовательных приближений
		...
		// Путем аппрокцисации переделываем решение под более крупную сетку
		...
		{// Метод итеративной проксимальной регуляризации
			...
			if (hNumber <= lastChangeH)
			{
				for (iA = 0; iA < nFE2; iA++)
					vOld[iA] = v[iA];

				n_ipr = (1 << (hNumber + 3)) + 1; // в соответствии с hNumber получаем 9, 17, 33, 65
	...
	if (outug.is_open())
	{
		for (int ix = 0; ix < nw; ix++)
		{
			outug << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] - v[0 * nFE + (nh - 1) * nw + ix];
			outug << "," << v[1 * nFE + 1 * nw * nh + ix] - v[1 * nFE + (nh - 1) * nw + ix] << ") ";
		}
		outug << std::endl << std::endl;
		for (int ix = 0; ix < nw; ix++)
			outug << "(" << ix * h + v[0 * nFE + 1 * nw * nh + ix] << "," << v[1 * nFE + 1 * nw * nh + ix] << ") ";
	}
	if (outl.is_open())
	{
...
void argminM(int n, double * tk, double * sk, double * t, double * s, double * l, double * vk, double * v, double * A, double * FP, double * g, long long r)
{
	...
	double ku; // Коэффициент для регуляризации
...
void makeA(int n, double * A, double koeffA[12][7][4])
{
	...
	{// Правая стенка верхнего тела не должна перемещаться вдоль горизонтальной оси
		...
		// Зануление строки и столбца и единица на диагонали
